...
f = open("log-domestic.txt", "a")
f.write("\n manipulating data(add day, month, year hour columns)--- %s seconds ---" % (time.time() - start_time))
f.close()
dataset.show()
...
# vectorize the fields that we want to use in KMeans
    ...
    f = open("log-domestic.txt", "a")
    f.write("\n vectorizing data --- %s seconds ---" % (time.time() - start_time))
    f.close()
...
# train data using Kmeans
        ...
        f = open("log-domestic.txt", "a")
        f.write("\n fit Kmeans --- %s seconds ---" % (time.time() - start_time))
        f.close()
# calculate the cluster for each data
        start_time = time.time()
        output = KMeans_fit.transform(data_scale_output)
        f = open("log-domestic.txt", "a")
        f.write("\n transform Kmeans --- %s seconds ---" % (time.time() - start_time))
        f.close()
# calculate silhouette score
        start_time = time.time()
        score = evaluator.evaluate(output)
        f = open("log-domestic.txt", "a")
        f.write("\n evaluate Kmeans --- %s seconds ---" % (time.time() - start_time))
        f.close()
